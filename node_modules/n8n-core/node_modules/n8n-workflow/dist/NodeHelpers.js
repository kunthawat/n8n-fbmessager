...
exports.isSingleExecution = isSingleExecution;
...
function convertNodeToAiTool(item) {
    ...
    if (isFullDescription(item.description)) {
        ...
        if (!item.description.properties.map((prop) => prop.name).includes('toolDescription')) {
            ...
            const noticeProp = {
                displayName: "Use the expression {{ $fromAI('placeholder_name') }} for any data to be filled by the model",
                name: 'notice',
                type: 'notice',
                default: '',
            };
...
function isSingleExecution(type, parameters) {
    const singleExecutionCase = Constants_1.SINGLE_EXECUTION_NODES[type];
    if (singleExecutionCase) {
        for (const parameter of Object.keys(singleExecutionCase)) {
            if (!singleExecutionCase[parameter].includes(parameters[parameter])) {
                return false;
            }
        }
        return true;
