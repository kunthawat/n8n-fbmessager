...
class Workflow {
        ...
        if (mode === 'manual') {
            ...
            triggerResponse.manualTriggerResponse = new Promise((resolve, reject) => {
                triggerFunctions.emit = ((resolveEmit) => (data, responsePromise, donePromise) => {
                    ...
                    if (donePromise) {
                        additionalData.hooks.hookFunctions.workflowExecuteAfter?.unshift(async (runData) => {
    ...
    async runWebhook(webhookData, node, additionalData, nodeExecuteFunctions, mode, runExecutionData) {
        const nodeType = this.nodeTypes.getByNameAndVersion(node.type, node.typeVersion);
        if (nodeType === undefined) {
        ...
        const context = nodeExecuteFunctions.getExecuteWebhookFunctions(this, node, additionalData, mode, webhookData, closeFunctions, runExecutionData);
    ...
    async runNode(executionData, runExecutionData, runIndex, additionalData, nodeExecuteFunctions, mode, abortSignal) {
        ...
        if (nodeType.execute) {
            ...
            const routingNode = new RoutingNode_1.RoutingNode(this, node, connectionInputData, runExecutionData ?? null, additionalData, mode);
            return {
