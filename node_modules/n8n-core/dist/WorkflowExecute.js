...
class WorkflowExecute {
    ...
    runPartialWorkflow2(workflow, runData, pinData = {}, dirtyNodeNames = [], destinationNodeName) {
        ...
        const trigger = (0, PartialExecutionUtils_1.findTriggerForPartialExecution)(workflow, destinationNodeName);
        if (trigger === undefined) {
            throw new n8n_workflow_1.ApplicationError('The destination node is not connected to any trigger. Partial executions need a trigger.');
        }
        let graph = PartialExecutionUtils_1.DirectedGraph.fromWorkflow(workflow);
        graph = (0, PartialExecutionUtils_1.findSubgraph)({ graph: (0, PartialExecutionUtils_1.filterDisabledNodes)(graph), destination, trigger });
        const filteredNodes = graph.getNodes();
        runData = (0, lodash_1.omit)(runData, dirtyNodeNames);
        let startNodes = (0, PartialExecutionUtils_1.findStartNodes)({ graph, trigger, destination, runData, pinData });
        startNodes = (0, PartialExecutionUtils_1.handleCycles)(graph, startNodes, trigger);
        runData = (0, PartialExecutionUtils_1.cleanRunData)(runData, graph, startNodes);
        const { nodeExecutionStack, waitingExecution, waitingExecutionSource } = (0, PartialExecutionUtils_1.recreateNodeExecutionStack)(graph, new Set(startNodes), runData, pinData ?? {});
        this.status = 'running';
        this.runExecutionData = {
            startData: {
